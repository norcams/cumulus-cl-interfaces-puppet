...
Puppet::Type.type(:cumulus_interface).provide :cumulus do
  ...
  def build_desired_config
    ...
    %w(bridge_allow_untagged mstpctl_portnetwork mstpctl_bpduguard mstpctl_portadminedge mstpctl_portbpdufilter clagd_enable clagd_priority
       clagd_backup_ip clagd_args clagd_sys_mac clagd_peer_ip clagd_vxlan_anycast_ip mtu gateway).each do |attr|
